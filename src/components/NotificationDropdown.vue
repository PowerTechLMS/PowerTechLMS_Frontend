<template>
  <div class="notification-container" ref="containerRef">
    <button class="btn-icon btn-ghost position-relative" @click="toggleDropdown">
      <Bell :size="20" />
    ...
    <Transition name="dropdown">
      <div v-if="isOpen" class="notification-dropdown glass shadow-lg">
        <div class="dropdown-header">
          <h3>Thông báo</h3>
          <button v-if="unreadCount > 0" class="btn-link" @click="markAllAsRead">Đánh dấu tất cả đã đọc</button>
        </div>
        ...
        <div class="dropdown-footer" v-if="notifications.length > 0">
          <button class="btn-ghost btn-sm w-full" @click="isOpen = false">Đóng</button>
        </div>
      </div>
    </Transition>
  </div>
...
import {
  Bell, BellOff, Loader2, BookOpen, Route,
...
const getIcon = (type) => {
  switch (type) {
    case 'EnrollmentRequest': return AlertCircle
    case 'EnrollmentStatus': return CheckCircle
    case 'NewCourse': return BookOpen
    case 'NewLearningPath': return Route
...
const getTypeClass = (type) => {
  switch (type) {
    case 'EnrollmentRequest': return 'warning'
    case 'EnrollmentStatus': return 'success'
    case 'NewCourse': return 'primary'
    case 'NewLearningPath': return 'info'
...
const startSignalR = async () => {
  if (connection.value) {
    await connection.value.stop().catch(err => console.error("SignalR stop error: ", err));
  }
...
.notification-message {
  ...
  display: -webkit-box;
  -webkit-line-clamp: 2;
  line-clamp: 2; /* Add standard for compatibility */
