    ...
    <aside class="sidebar" :class="{ collapsed: sidebarCollapsed, 'mobile-open': mobileMenuOpen }">
      <div class="sidebar-header">
        <div class="logo" @click="$router.push('/')">
          <div class="logo-icon">
            <svg width="28" height="28" viewBox="0 0 28 28" fill="none" style="display:block;width:28px;height:28px;">
      ...
      <div class="dlabnav">
        <div class="dlabnav-scroll custom-scrollbar">
          <ul class="doc-metismenu" id="menu">

            <li class="nav-label">CHUNG</li>

            <li>
              <RouterLink to="/" :class="{ active: $route.name === 'Dashboard' }" @click="mobileMenuOpen = false">
            ...
            <li>
              <RouterLink to="/leaderboard" :class="{ active: $route.path.startsWith('/leaderboard') }" @click="mobileMenuOpen = false">
            ...
            <template v-if="authStore.canManage">
              ...
              <li v-if="authStore.isAdmin || authStore.isInstructor || authStore.hasPermission('course.view')">
                <RouterLink to="/admin/courses"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-training"><BookOpen :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasPermission('group.manage')">
                <RouterLink to="/admin/coursegroup"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-training"><Route :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.isInstructor || authStore.hasPermission('enrollment.view')">
                <RouterLink to="/admin/enrollments"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-training"><Users :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.isInstructor || authStore.hasPermission('quiz.manage')">
                <RouterLink to="/admin/quiz-analysis"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-training"><BarChart3 :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasPermission('user.manage')">
                <RouterLink to="/admin/departments"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-org"><Building2 :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasPermission('user.manage')">
                <RouterLink to="/admin/staff"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-org"><UserCog :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasAnyPermission('user.manage', 'doc.view', 'doc.upload', 'doc.delete')">
                <RouterLink to="/admin/documents"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-org"><FolderOpen :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasAnyPermission('certificate.view', 'certificate.manage')">
                <RouterLink to="/admin/certificates"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-org"><Award :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasPermission('report.view')">
                <RouterLink to="/admin/reports"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-system"><PieChart :size="15" /></span>
              ...
              <li v-if="authStore.isAdmin || authStore.hasPermission('role.manage')">
                <RouterLink to="/admin/rbac"
                  ...
                  @click="mobileMenuOpen = false">
                  <span class="admin-item-icon icon-system"><KeyRound :size="15" /></span>
      ...
      <header class="main-header glass">
        ...
        <div class="mobile-logo mobile-only" @click="$router.push('/')">
          <svg width="24" height="24" viewBox="0 0 28 28" fill="none">
            ...
            <path d="M8 14L12 18L20 10" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"/>
            <defs>
              <linearGradient id="logo-gradient-m" x1="0" y1="0" x2="28" y2="28">
                <stop stop-color="#6366f1"/>
                <stop offset="1" stop-color="#a855f7"/>
              </linearGradient>
            </defs>
        ...
        <div class="header-actions">
          ...
          <div class="user-menu" @click="showUserMenu = !showUserMenu" ref="userMenuRef">
            ...
            <div v-else class="user-avatar">{{ authStore.user?.fullName?.charAt(0) }}</div>
            <span class="user-name desktop-tablet-only">{{ authStore.user?.fullName }}</span>
            <ChevronDown :size="14" class="desktop-tablet-only" />

            <Transition name="dropdown">
              <div class="user-dropdown" v-if="showUserMenu" @click.stop>
                <div class="dropdown-header">
                  <div v-if="avatarUrl" class="user-avatar lg" :style="{ backgroundImage: `url('${avatarUrl}')`, backgroundSize: 'cover', backgroundPosition: 'center', color: 'transparent' }"></div>
                  <div v-else class="user-avatar lg">{{ authStore.user?.fullName?.charAt(0) }}</div>
                  <div>
                    <div class="dropdown-name">{{ authStore.user?.fullName }}</div>
                    <span class="badge" :class="'badge-' + roleBadge">{{ authStore.user?.role }}</span>
                  </div>
                </div>
...
const syncSidebarGroups = () => {
  ...
  if (path === '/courses' || path.startsWith('/my-courses')) openGroups.value.learning = true
  if ((path.startsWith('/documents') || path.startsWith('/certificates')) && !path.startsWith('/admin')) {
    openGroups.value.resources = true
  }
  
  // Sửa logic để Khung lộ trình tự động mở tab Training
  if (path.startsWith('/admin')) {
    if (path.includes('/courses') || path.includes('/enrollments') || path.includes('/quiz-analysis') || path.includes('/learning-paths')) {
        openGroups.value.adminTraining = true
    }
    if (path.includes('/departments') || path.includes('/admin/documents') || path.includes('/admin/certificates') || path.includes('/staff')) {
...
.sidebar.collapsed:hover .toggle-btn.desktop-only { display: flex; }
...
/* Second label (QUẢN LÝ) with accent accent */
.nav-label-admin {
...
.sidebar.collapsed .doc-has-arrow::after { display: none; }
...
.doc-metismenu ul li a::after { display: none !important; }
...
.sidebar.collapsed .doc-metismenu ul { display: none !important; }
...
@media (max-width: 1024px) { .header-search { width: 200px; } }
...
.user-menu:hover { background: var(--bg-glass); }
...
.user-avatar.lg { width: 40px; height: 40px; font-size: var(--font-size-md); }
...
.desktop-tablet-only { display: flex; }
...
/* ===================== Admin Nav Items (flat, with icon box) ===================== */
.admin-nav-item {
  display: flex;
  align-items: center;
  gap: 10px;
...
.sidebar.collapsed .admin-nav-item .nav-text { display: none; }
.sidebar.collapsed .admin-group-header,
.sidebar.collapsed .admin-group-divider { display: none; }
</style>