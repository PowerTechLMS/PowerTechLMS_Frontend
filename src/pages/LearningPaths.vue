  ...
  <div class="learning-paths-page">
    <div class="container py-5 mt-4">
      ...
      <div class="page-header mb-5 animate-slide-up">
        <div class="d-flex align-items-center gap-4">
          ...
          <div>
            ...
            <p class="page-desc">Chương trình đào tạo được thiết kế bài bản giúp bạn chinh phục các kỹ năng chuyên môn.</p>
          </div>
        </div>
      </div>
      ...
      <div class="row g-4 mb-5 animate-slide-up" style="animation-delay: 0.1s">
        <div class="col-md-3">
          <div class="glass-stat-card primary">
            <div class="stat-icon"><Layers :size="24" /></div>
        ...
        <div class="col-md-3">
          <div class="glass-stat-card info">
            ...
            <div class="stat-content">
              ...
              <div class="stat-value">{{ totalCoursesInPaths }}</div>
            </div>
          </div>
        </div>
        <div class="col-md-3">
          <div class="glass-stat-card success">
            <div class="stat-icon"><Award :size="24" /></div>
            <div class="stat-content">
              <div class="stat-label">CHỨNG CHỈ ĐANG CHỜ</div>
              <div class="stat-value">{{ totalPendingCertificates }}</div>
            </div>
          </div>
        </div>
        <div class="col-md-3">
      ...
      <div v-else-if="learningPaths.length === 0" class="empty-state text-center py-5 glass animate-slide-up">
        ...
        <p class="text-muted">Bộ phận nhân sự sẽ cập nhật lộ trình học tập cho bạn sớm thôi!</p>
        <button @click="$router.push('/courses')" class="btn-neon mt-3">KHÁM PHÁ THƯ VIỆN</button>
      </div>
          ...
          <div class="path-card glass" @click="viewPathDetail(path.id)">
            <div class="path-card-header">
              <div class="path-icon-wrap">
                <div class="path-icon"><Route :size="24" /></div>
              </div>
            ...
            <div class="path-card-body p-4">
              <h3 class="path-title mb-2">{{ path.name }}</h3>
              <p class="path-desc text-truncate-2">{{ path.description || 'Chương trình đào tạo chuyên sâu dành cho nhân sự tiềm năng.' }}</p>
              ...
              <div class="path-meta mt-4 py-3 border-top border-bottom">
                ...
                <div class="path-progress-wrap">
                  ...
                  <div class="premium-progress-bar">
                    <div class="progress-fill" :style="{ width: getPathProgress(path) + '%' }"></div>
                  </div>
                </div>
              </div>
              ...
              <div class="path-card-footer mt-4 d-flex align-items-center justify-content-between">
                <div class="avatars-group d-flex">
                  <div v-for="i in 3" :key="i" class="mini-avatar" :style="{ zIndex: 4-i, transform: `translateX(${(i-1)*-10}px)` }">
                    <img :src="`https://i.pravatar.cc/100?u=${path.id + i}`" alt="user" />
                  </div>
                  <div class="plus-more" :style="{ transform: `translateX(-20px)` }">+12</div>
                </div>
...
import { 
  Route, Layers, BookOpen, Award, Clock, 
  ChevronRight, Users 
...
onMounted(async () => {
  loading.value = true;
  try {
    // Tải song song lộ trình và tiến độ ghi danh
...
const totalCoursesInPaths = computed(() => {
  return learningPaths.value.reduce((sum, path) => sum + (path.courseCount || 0), 0);
});

const totalPendingCertificates = computed(() => {
  return learningPaths.value.reduce((sum, path) => sum + (path.pendingCertCount || 0), 0);
});

const overallProgress = computed(() => {
  if (learningPaths.value.length === 0) return 0;
  const total = learningPaths.value.reduce((sum, path) => sum + getPathProgress(path), 0);
...
.glass-stat-card.primary .stat-icon { background: rgba(79, 70, 229, 0.1); color: #4f46e5; }
.glass-stat-card.info .stat-icon { background: rgba(14, 165, 233, 0.1); color: #0ea5e9; }
.glass-stat-card.success .stat-icon { background: rgba(16, 185, 129, 0.1); color: #10b981; }
...
.path-badge.primary { background: #4f46e5; color: white; }
.path-badge.info { background: #0ea5e9; color: white; }
