<template>
	...
	>
		...
		<div v-else-if="error" class="player-error card glass">
			<AlertCircle :size="48" style="color:var(--danger-400,#f87171);opacity:.6;margin-bottom:1rem" />
			<h3>{{ error }}</h3>
					...
					<div class="progress-stats">
						<div class="stats-text">
							...
							<span class="lesson-count-text ms-2">
								...
								{{ course?.modules?.flatMap(m => m.lessons).length || 0 }} bài)
							</span>
						</div>
						<div class="progress-mini-bar">
							<div
								class="progress-mini-fill"
								:style="{ width: Math.round(courseProgress?.progressPercent || 0) + '%' }"
							></div>
						</div>
					</div>
					<button
			...
			<div class="player-container">
							...
							<div class="lesson-list" v-show="expandedModules.has(mod.id)">
								<template v-for="l in mod.lessons" :key="l.id">
									...
									<div
										...
										:aria-current="l.id === lesson?.id && activeTab !== 'quiz' ? 'page' : undefined"
									...
									>
										<div class="lesson-status">
											<div v-if="!canAccess(l)" class="status-icon locked" title="Bài học đang đóng">
												<Lock :size="14" />
											</div>
											<div v-else-if="isLessonDone(l.id)" class="status-icon completed" title="Đã hoàn thành">
												<Check :size="16" />
											</div>
											<div v-else-if="l.id === lesson?.id" class="status-icon active" title="Đang học">
										...
										<div class="lesson-info">
											...
											<div class="l-meta">
												<div class="l-type-tag">
													...
													<span>{{ l.type === 'Video' ? 'Video' : 'Bài đọc' }}</span>
												</div>
												<span class="l-duration" v-if="l.type === 'Video' && l.videoDurationSeconds">
													<Clock :size="12" />
													{{ formatTime(l.videoDurationSeconds) }}
												</span>
												<span class="l-progress-tag" v-if="l.type === 'Video' && getLessonProgressPercent(l.id) > 0 && !isLessonDone(l.id)">
						...
						<template v-if="course?.extraQuizzes?.length">
							...
							>
								<div
									...
									:class="{ completed: courseProgress?.passedQuizIds?.includes(q.id) }"
									role="button"
									tabindex="0"
									:aria-label="`Kiểm tra chung: ${q.title}`"
									@click="router.push(`/quiz/${q.id}?courseId=${course.id}`)"
									@keydown.enter.space.prevent="router.push(`/quiz/${q.id}?courseId=${course.id}`)"
								>
									<div class="lesson-status">
										<CheckCircle
									...
									<div class="lesson-info">
										...
										<span class="l-meta text-warning fw-bold">Kiểm tra chung</span>
									</div>
								</div>
							</div>
						</template>
				...
				<main class="player-content">
					<div class="content-primary">
						...
						<div class="video-theater glass" v-if="lesson?.type === 'Video'">
							<div class="video-container">
								...
								<div v-if="!isCompleted && videoDurationRef > 0" class="video-watch-indicator" :title="`Đã xem ${videoWatchPercent}%`">
									<svg width="40" height="40" viewBox="0 0 40 40">
										...
										/>
										<text x="20" y="20" text-anchor="middle" dominant-baseline="central" fill="white" font-size="9" font-weight="600">
											{{ videoWatchPercent }}%
										</text>
									</svg>
								</div>
								...
								<template v-else>
									...
									>
										...
										<h3 class="processing-title">Bài giảng đang được xử lý...</h3>
										<p class="processing-sub">Vui lòng chờ giây lát, hệ thống đang tối ưu video cho bạn.</p>
									</div>
									<video
										v-else-if="lesson?.videoUrl"
										:src="(lesson.videoStatus === 'Ready' && lesson.videoUrl.endsWith('.m3u8')) ? null : getFullMediaUrl(lesson.videoUrl)"
										ref="videoRef"
										controls
										...
										@seeked="onSeeked"
									></video>
									<div v-else class="no-video flex-center flex-column gap-3 p-5">
						...
						<div
							v-if="lesson?.type === 'Text' && !isCompleted && !isReadingTimeFinished"
						...
						>
							<div class="reading-timer-inner">
								...
								<div class="reading-timer-info">
									<span class="reading-timer-label">Thời gian đọc tối thiểu</span>
									<span class="reading-timer-remain">Còn {{ readingTimeLeft }}s</span>
								</div>
									...
									<div class="lesson-intro">
										<div class="intro-header-modern">
											<div class="title-area">
												...
												<p class="text-secondary mt-1 mb-0" v-if="lesson?.type === 'Video'">
													<Clock :size="14" class="me-1" />
													Thời lượng: {{ formatTime(lesson?.videoDurationSeconds) }}
												</p>
												<p class="text-secondary mt-1 mb-0" v-else-if="lesson?.type === 'Text'">
													<Clock :size="14" class="me-1" />
													Thời gian đọc kiến nghị: {{ formatTime(lesson?.readingDurationSeconds || 180) }}
												</p>
											</div>
										...
										<div class="lesson-description-structure mt-4">
											<div class="desc-section">
												...
												<div class="section-body">
													<p v-if="!lesson?.description" class="text-muted opacity-75">
														Mục tiêu bài học này là giúp bạn nắm vững các kiến thức nền tảng và ứng dụng thực tế của chủ đề hiện tại.
													</p>
													<div v-else v-html="lesson.description"></div>
												</div>
											</div>

											<div class="desc-section mt-4" v-if="lesson?.content && lesson?.type === 'Video'">
									...
									<div v-else-if="quizStarted && !quizResult" class="quiz-active-area">
										...
										>
											<h6 class="fw-bold text-dark lh-base mb-4">
												<span class="text-primary me-2">Câu {{ index + 1 }}:</span>
												{{ q.questionText }}
									...
									<div v-else-if="quizResult" class="text-center py-5">
										...
										<div v-else>
											...
											<div class="fs-5 text-muted mb-3">
												Điểm của bạn:
												<strong class="text-dark fs-3">{{ quizResult.score }}</strong>
												/ {{ quizResult.totalScore }}
											</div>

											<!-- FIX: Hiển thị countdown trực quan -->
											<div v-if="quizRetryCountdown > 0" class="retry-countdown-box mb-4">
												<div class="countdown-inner">
													<Clock :size="20" />
													<span>Bạn có thể làm lại sau</span>
													<strong class="countdown-num">{{ formatRetryCountdown(quizRetryCountdown) }}</strong>
												</div>
												<div class="countdown-track">
													<div
														class="countdown-fill"
														:style="{ width: (1 - quizRetryCountdown / 300) * 100 + '%' }"
													></div>
												</div>
											</div>
											...
											<div class="d-flex justify-content-center gap-3">
												...
												>
													...
													{{ quizRetryCountdown > 0 ? `Chờ ${formatRetryCountdown(quizRetryCountdown)}` : 'Làm lại bài' }}
												</button>
											</div>
										</div>

										<!-- Chi tiết bài làm -->
										<div class="mt-5 text-start" v-if="quizResult.details && quizResult.details.length">
											<h4 class="fw-bold mb-4 border-bottom pb-2 text-dark">Chi tiết bài làm</h4>
											<div v-for="(d, i) in quizResult.details" :key="i" class="card mb-3 border-0 shadow-sm" :class="d.isCorrect ? 'border-start border-success' : 'border-start border-danger'" style="border-left-width: 5px !important;">
												<div class="card-body">
													<p class="fw-bold fs-15 text-dark mb-2">Câu {{ i + 1 }}: {{ d.questionText }}</p>
													<div class="d-flex flex-wrap gap-4 fs-14">
														<span :class="d.isCorrect ? 'text-success fw-bold' : 'text-danger fw-bold'">
															Bạn chọn: {{ d.selectedAnswer ? getOptionText(d.questionId, d.selectedAnswer) : 'Bỏ trống' }}
														</span>
														<span v-if="!d.isCorrect" class="text-success fw-bold">
															Đáp án đúng: {{ getOptionText(d.questionId, d.correctAnswer) }}
														</span>
													</div>
													<div v-if="d.explanation && d.explanation.trim() !== ''" class="mt-3 p-3 bg-light rounded fs-14 text-dark border">
														<strong class="text-primary"><HelpCircle :size="16" class="me-1" style="margin-top:-3px"/> Giải thích:</strong> {{ d.explanation }}
													</div>
												</div>
											</div>
										</div>
										...
										<div class="qa-list" v-if="qaList.length">
											<div v-for="qa in qaList" :key="qa.id" class="qa-card card glass">
												<div class="qa-main">
													<div
														class="user-avatar"
														:style="qa.userAvatar ? { backgroundImage: `url('${getAvatarUrl(qa.userAvatar)}')` } : {}"
													...
													<div class="qa-body">
														<div class="qa-meta">
															...
															<span
																v-if="qa?.userRole === 'Admin' || qa?.userRole === 'Instructor'"
														...
														<button
															...
															@click="replyTo = replyTo === qa.id ? null : qa.id"
														>
															Trả lời
														</button>
													</div>
												...
												<div class="replies-section" v-if="qa.replies?.length || replyTo === qa.id">
													...
													<div v-for="reply in qa.replies" :key="reply.id" class="reply-item">
														<div
															...
															:style="reply.userAvatar ? { backgroundImage: `url('${getAvatarUrl(reply.userAvatar)}')` } : {}"
														...
														<div class="qa-body">
															<div class="qa-meta">
																...
																<span
																	v-if="reply?.userRole === 'Admin' || reply?.userRole === 'Instructor'"
																	class="badge bg-primary ms-1"
																	style="font-size:9px;padding:2px 6px"
																>GIẢNG VIÊN</span>
										...
										<div class="notes-list" v-if="notes.length">
											<div v-for="note in notes" :key="note.id" class="note-item card glass">
												<div
													...
													@keydown.enter.space.prevent="seekVideo(note.videoTimestampSeconds)"
												>
													<Video :size="12" />
					...
					<div class="content-footer-nav glass">
						<button class="btn btn-ghost" @click="navigatePrev" :disabled="!prevLesson" aria-label="Bài học trước">
							<ChevronLeft :size="18" /> Bài trước
						</button>
						<div class="footer-lesson-info hide-mobile">
							Đang xem: <span>{{ lesson?.title }}</span>
						</div>
						<button class="btn btn-primary" @click="goToNextLesson" v-if="nextLesson" aria-label="Bài học tiếp theo">
							Bài tiếp theo <ChevronRight :size="18" />
						</button>
						<button
							class="btn btn-success"
							v-else-if="course?.finalQuizId"
							@click="router.push(`/quiz/${course.finalQuizId}?courseId=${course.id}`)"
						>
							Làm bài thi cuối khóa <ClipboardList :size="18" />
						</button>
					</div>
				</main><!-- /main.player-content -->
		...
		<teleport to="body">
			<div v-if="showCourseCompletedModal" class="glass-modal-overlay" role="dialog" aria-modal="true" aria-labelledby="modal-title">
				<div class="congratulations-box p-5 rounded bg-white text-center shadow-lg animate-slide-up" style="max-width:500px;z-index:9999">
					...
					<h2 id="modal-title" class="text-success fw-bold mb-3">🎉 HOÀN THÀNH KHÓA HỌC! 🎉</h2>
					<p class="text-secondary mb-4 fs-15">
						Bạn đã hoàn thành xuất sắc toàn bộ nội dung của khóa học này. Hệ thống đang tiến hành đánh giá và lưu trữ thành tích.
					</p>
					...
					<div class="d-flex justify-content-center gap-3 mt-4">
						<button class="btn btn-outline-secondary px-4 fw-bold" @click="showCourseCompletedModal = false">
							Đóng & Xem kết quả
						</button>
						<button
		...
		<transition name="fade">
			<div v-if="lessonLoading" class="lesson-switch-overlay" aria-live="polite" aria-label="Đang chuyển bài học">
...
import {
	...
	ClipboardList, Paperclip, MessageCircle, StickyNote, Trash2,
	Download, X, LayoutList, Video, FileText, Circle, Info,
	HelpCircle, Trophy, RotateCcw, Award, PlayCircle, Maximize,
	Minimize, Check, VideoOff, Play, Lock, Clock, ArrowRight,
...
const isCompleted = computed(() =>
	lessonProgresses.value.some(lp => lp.lessonId === lesson.value?.id && lp.isCompleted)
);

const isCurrentLessonQuizPassed = computed(() => {
	const currentLP = lessonProgresses.value.find(lp => lp.lessonId === lesson.value?.id);
	return currentLP?.isQuizPassed || false;
});

const canCompleteManual = computed(() => {
	if (lesson.value?.type === 'Video' && !isVideoWatchedEnough.value) return false;
	if (lesson.value?.hasQuiz && !isCurrentLessonQuizPassed.value) return false;
	if (lesson.value?.type === 'Text' && !isReadingTimeFinished.value) return false;
	return true;
});
...
const videoWatchPercent = computed(() => {
	...
	return Math.min(100, Math.round((maxWatchedTime.value / videoDurationRef.value) * 100));
});
...
async function loadData() {
	...
	try {
		...
		if (!canAccess(targetLesson)) {
			toast.warning('🔒 Bài học này đang bị khóa. Bạn cần hoàn thành các bài học trước đó để mở khóa nội dung này.');
		...
		if (quizRetryInterval) { clearInterval(quizRetryInterval); quizRetryInterval = null; }
...
// ────────────────────────────────────────────
function setupHls() {
	if (!videoRef.value || !lesson.value?.videoUrl || !lesson.value.videoUrl.endsWith('.m3u8')) return;
...
async function updateSignalRLessonGroup() {
	...
	if (signalRCurrentLessonId && signalRCurrentLessonId !== lesson.value?.id) {
		try { await signalrConnection.invoke('LeaveLessonGroup', signalRCurrentLessonId); } catch {}
	}
	if (lesson.value) {
		try {
...
function onSeeked(e) {
	...
	if (video.currentTime < preSeekTime.value) { lastCurrentTime.value = video.currentTime; return; }
	if (video.currentTime <= maxWatchedTime.value + 2) { lastCurrentTime.value = video.currentTime; return; }
...
function onTimeUpdate(e) {
	...
	if (!videoDurationRef.value && video.duration) videoDurationRef.value = video.duration;
	...
	if (!isCompleted.value && video.duration > 0 && current >= video.duration * 0.9) {
...
async function startMiniQuiz() {
	...
	try {
		...
		if (data.remainingAttemptsToday !== undefined) {
			toast.info(`Bắt đầu bài tập. Bạn còn ${data.remainingAttemptsToday} lượt làm bài trong hôm nay.`);
		}
	} catch (e) {
...
async function submitMiniQuiz() {
	...
	const answerList = Object.entries(selectedAnswers.value).map(([questionId, selectedAnswer]) => ({
	...
	try {
		...
		if (data.isPassed) {
			...
			const remainingText = data.remainingAttemptsToday > 0
				? ` Bạn còn ${data.remainingAttemptsToday} lần làm lại trong hôm nay.`
				: ' Bạn đã hết lượt thi trong hôm nay.';
			toast.warning(`Rất tiếc, bạn chưa đạt điểm. Theo quy định, bạn cần ôn tập và chờ 5 phút để làm lại.${remainingText}`);
...
async function completeLesson(isQuizPassedArg = false) {
	const isQuizPassed = typeof isQuizPassedArg === 'boolean' ? isQuizPassedArg : false;
	...
	try {
		...
		if (err.response?.status === 400 && err.response.data?.errors) {
			...
			toast.error(err.response?.data?.message || 'Lỗi khi ghi nhận hoàn thành bài học.');
		}
	} finally {
...
function navigateLesson(l, targetTab = 'overview') {
	...
	if (String(l.id) === String(lesson.value?.id) && targetTab === activeTab.value) return;
...
function getOptionText(questionId, optionKey) {
	if (!lesson.value || !lesson.value.quiz || !lesson.value.quiz.questions || !optionKey) return optionKey;
...
.stats-text span:first-child { color: var(--text-primary, #0f172a); font-weight: 800; }
...
.module-item:hover,
.module-item:focus-visible { background: rgba(15, 23, 42, 0.04); outline: none; }
...
.module-index { font-weight: 600; text-transform: uppercase; letter-spacing: 0.04em; }
.module-title { font-size: var(--font-size-sm, 13px); font-weight: 600; margin: 0 0 4px; color: var(--text-primary, #0f172a); }
.module-stats { font-size: 10px; color: var(--text-tertiary, #94a3b8); }
.lesson-list { background: rgba(15, 23, 42, 0.02); }
...
.lesson-item.active { background: rgba(99, 102, 241, 0.08); border-left-color: var(--primary-400, #6366f1); }
.lesson-item.completed { background: rgba(16, 185, 129, 0.04); }
.lesson-item.locked { opacity: 0.6; cursor: default; }
...
.status-icon.completed { background: rgba(16, 185, 129, 0.15); color: #10b981; }
.status-icon.active { background: var(--primary-400, #818cf8); color: white; box-shadow: 0 0 10px rgba(99, 102, 241, 0.4); }
.status-icon.todo { background: rgba(15,23,42,0.04); color: var(--text-tertiary, #94a3b8); border: 1.5px solid var(--border-color); }
.status-icon.locked { background: rgba(15,23,42,0.04); color: var(--text-tertiary, #94a3b8); opacity: 0.5; }

.lesson-info { flex: 1; min-width: 0; }
.l-title { font-weight: 600; font-size: 13.5px; color: var(--text-primary, #0f172a); line-height: 1.4; display: block; margin-bottom: 4px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.l-meta { display: flex; align-items: center; gap: 10px; flex-wrap: wrap; }
.l-type-tag { display: flex; align-items: center; gap: 4px; font-size: 10px; font-weight: 700; color: var(--text-tertiary, #94a3b8); text-transform: uppercase; }
.l-duration { display: flex; align-items: center; gap: 4px; font-size: 10.5px; color: var(--text-tertiary, #94a3b8); font-variant-numeric: tabular-nums; }
.l-progress-tag { font-size: 10px; color: var(--primary-400, #6366f1); font-weight: 700; background: rgba(99, 102, 241, 0.1); padding: 1px 6px; border-radius: 4px; }
.done-icon { color: var(--success-400, #4ade80); }
.todo-icon { color: var(--border-color); }
.quiz-sub-item { background: rgba(99, 102, 241, 0.03); }
.final-quiz-item { background: rgba(245, 158, 11, 0.05); border-left: 3px solid transparent; }
.final-quiz-item.active { background: rgba(245, 158, 11, 0.15); border-left-color: var(--warning-500, #f59e0b); }
...
.content-primary {
	flex: 1;
	max-width: min(1000px, calc(100vw - clamp(280px, 28vw, 360px) - 48px)); /* FIX: Tính đúng max-width */
	margin: 0 auto;
	width: 100%;
...
.content-type-badge.video { border-color: rgba(99, 102, 241, 0.3); }
.content-type-badge.text { border-color: rgba(16, 185, 129, 0.3); position: relative; top: auto; left: auto; margin: 16px; display: inline-flex; }
...
.reading-timer-icon { color: var(--primary-400, #6366f1); flex-shrink: 0; }
...
.reading-timer-label { font-size: 11px; color: var(--text-tertiary, #94a3b8); }
.reading-timer-remain { font-size: 13px; font-weight: 700; color: var(--primary-400, #6366f1); }
...
.processing-glow { width: 80px; height: 80px; border-radius: 50%; background: var(--gradient-primary); filter: blur(20px); animation: pulse-glow 2s infinite alternate; }
@keyframes pulse-glow { from { transform: scale(0.8); opacity: 0.5; } to { transform: scale(1.2); opacity: 0.8; } }
.processing-title { font-weight: 900; margin-bottom: 8px; font-size: 24px; }
.processing-sub { color: rgba(255,255,255,0.6); font-size: 14px; }
...
.tab-btn:hover { color: var(--text-primary, #0f172a); }
.tab-btn.active { color: var(--primary-400, #6366f1); font-weight: 600; }
.tab-btn.active::after { content: ''; position: absolute; bottom: -1px; left: var(--space-lg, 16px); right: var(--space-lg, 16px); height: 3px; background: var(--primary-500, #6366f1); border-radius: 3px 3px 0 0; }

.tab-badge { font-size: 10px; background: var(--bg-tertiary, #f1f5f9); padding: 1px 6px; border-radius: 10px; }
...
.section-title-dot::before { content: ''; width: 6px; height: 6px; background: var(--primary-400, #818cf8); border-radius: 50%; }

.section-body { color: var(--text-secondary, #475569); line-height: 1.7; font-size: 14.5px; }
.section-body :deep(ul), .section-body :deep(ol) { padding-left: 20px; margin: 12px 0; }
.section-body :deep(li) { margin-bottom: 8px; }
...
.completed-area-modern { display: flex; align-items: center; flex-wrap: wrap; gap: 12px; }
...
.res-icon { color: var(--primary-400, #6366f1); }
.res-info { flex: 1; }
.res-name { font-size: var(--font-size-sm, 13px); font-weight: 600; color: var(--text-primary, #0f172a); }
.res-meta { font-size: 10px; color: var(--text-tertiary, #94a3b8); }
.res-dl { color: var(--text-tertiary, #94a3b8); }
...
.empty-state { display: flex; flex-direction: column; align-items: center; gap: 12px; padding: 48px 24px; color: var(--text-tertiary, #94a3b8); }

/* ================================================================
   TAB: Q&A
================================================================ */
.qa-container { display: flex; flex-direction: column; gap: var(--space-xl, 24px); }
.qa-input-box { padding: var(--space-lg, 16px); border-radius: var(--radius-lg, 12px); }
...
.qa-input-box textarea { height: 100px; margin-bottom: var(--space-sm, 8px); }
.qa-actions { display: flex; justify-content: flex-end; }
.qa-list { display: flex; flex-direction: column; gap: var(--space-lg, 16px); }
.qa-card { padding: var(--space-lg, 16px); border-radius: var(--radius-lg, 12px); }
.qa-main { display: flex; gap: var(--space-md, 12px); }
...
.qa-body { flex: 1; }
.qa-meta { font-size: 11px; color: var(--text-tertiary, #94a3b8); margin-bottom: 4px; display: flex; align-items: center; gap: 4px; flex-wrap: wrap; }
.qa-meta strong { color: var(--text-primary, #0f172a); }
.qa-text { font-size: var(--font-size-sm, 13px); line-height: 1.6; color: var(--text-secondary, #475569); margin-bottom: 8px; }
.reply-btn { background: none; border: none; color: var(--primary-400, #6366f1); font-size: 12px; font-weight: 600; cursor: pointer; padding: 0; }

.replies-section { margin-top: var(--space-md, 12px); padding-left: var(--space-xl, 24px); border-left: 2px solid var(--border-color); display: flex; flex-direction: column; gap: var(--space-lg, 16px); }
.reply-item { display: flex; gap: var(--space-md, 12px); }
.reply-input-wrap { display: flex; flex-direction: column; gap: var(--space-xs, 4px); margin-bottom: var(--space-md, 12px); }
.reply-input-wrap textarea { height: 60px; }

/* ================================================================
   TAB: NOTES
================================================================ */
.notes-container { display: flex; flex-direction: column; gap: var(--space-xl, 24px); }
...
.note-input textarea { height: 100px; }
.notes-list { display: flex; flex-direction: column; gap: var(--space-md, 12px); }
...
.note-time:hover,
.note-time:focus-visible { background: var(--primary-500, #6366f1); box-shadow: 0 0 15px rgba(99, 102, 241, 0.4); outline: none; }

.note-item p { font-size: var(--font-size-sm, 13px); color: var(--text-secondary, #475569); line-height: 1.6; margin: 0; }
...
.delete-note:hover { color: var(--danger-400, #f87171); }
...
.footer-lesson-info { font-size: var(--font-size-sm, 13px); color: var(--text-tertiary, #94a3b8); }
.footer-lesson-info span { color: var(--text-primary, #0f172a); font-weight: 600; margin-left: var(--space-sm, 8px); }
...
.custom-scrollbar::-webkit-scrollbar-thumb:hover { background: var(--text-tertiary, #94a3b8); }
...
.card { background: var(--bg-card, #ffffff); border: 1px solid var(--border-color); border-radius: var(--radius-lg, 12px); }

/* Focus mode button */
.focus-mode-btn { color: var(--text-tertiary, #94a3b8) !important; }
.focus-mode-btn:hover { background: rgba(255,255,255,0.1) !important; color: white !important; }

/* ================================================================
   RESPONSIVE
================================================================ */
@media (max-width: 1200px) {
	.content-primary { max-width: calc(100% - 32px); }
}
...
@media (max-width: 1024px) {
	...
	.content-primary { max-width: 100%; padding: var(--space-md, 12px); }
}

@media (max-width: 768px) {
	.player-header-bar { padding: 0 var(--space-sm, 8px); }
	.course-name { max-width: 150px; font-size: var(--font-size-sm, 13px); }
	.course-info { display: none; } /* Thu gọn trên mobile nhỏ */
	...
	.tab-btn { padding: var(--space-md, 12px) var(--space-sm, 8px); font-size: 12px; }
	.intro-header-modern { flex-direction: column; }
	.action-area { width: 100%; }
	.btn-complete-premium, .btn-next-direct { width: 100%; justify-content: center; }
}

@media (max-width: 480px) {
	/* Chỉ hiện icon trên mobile rất nhỏ */
	.tab-btn span { display: none; }
	.tab-btn { padding: var(--space-md, 12px); }
}
</style>