<template>
	<div class="verify-page">
		<div class="bg-decor deco-1" />
		<div class="bg-decor deco-2" />

		<div class="verify-container">
			<div
				v-if="loading"
				class="verify-status-card glass animate-fade-in text-center py-5"
			>
				<div class="premium-spinner mx-auto mb-4" />
				<h3 class="fw-bold text-dark">Đang kiểm tra dữ liệu...</h3>
				<p class="text-tertiary">
					Vui lòng chờ trong giây lát để hệ thống tra cứu mã xác thực.
				</p>
			</div>

			<div
				v-else-if="notFound"
				class="verify-status-card glass danger animate-fade-in text-center py-5"
			>
				...
				<p class="text-secondary mb-4">
					Rất tiếc, mã xác thực <strong>{{ route.params.code }}</strong> không
			...
			<div
				v-else-if="cert && cert.status === 'Revoked'"
				class="verify-status-card glass warning animate-fade-in text-center py-5"
			>
				...
				<p class="text-secondary mb-4">
					Chứng chỉ mang mã <strong>{{ cert.certificateCode }}</strong> đã bị vô
					hiệu hóa bởi Ban Quản trị.
				</p>
				<button class="btn-glass-primary" @click="router.push('/')">
					<Home :size="18" class="me-2" /> Về trang chủ
				</button>
			...
			<div
				v-else-if="
					cert && (cert.status === 'Issued' || cert.status === 'Valid')
				"
				class="success-container animate-fade-in"
			>
				<div class="status-banner valid mb-4 shadow-sm">
					...
					<div class="text-start">
						...
						<p class="text-dark mb-0 fs-15">
							Hệ thống xác nhận mã
							<strong>{{ cert.certificateCode }}</strong> là hoàn toàn hợp lệ.
						</p>
					</div>
				</div>
						...
						<div class="cert-modern-border-outer">
							<div class="cert-modern-border-inner">
								...
								<div class="cert-content">
									...
									<h3 class="cert-course-name">
										{{ cert.courseTitle }}
									</h3>

									<div class="cert-footer">
										<div class="cert-footer-col">
											<p class="cert-val text-dark">
										...
										<div class="cert-footer-col">
											<p class="cert-val signature-font">
												{{ templateConfig.SignatureName }}
											</p>
											<div class="cert-signature-line" />
											<p class="cert-label">
												{{ templateConfig.SignatureTitle }}
											</p>
										</div>
									</div>
								</div>
			...
			<div v-else-if="cert" class="success-container animate-fade-in">
				<p class="text-muted">
					Đang hiển thị chứng chỉ (Status: {{ cert.status }})
				</p>
			</div>
		</div>
	</div>
...
const downloadAsImage = () => {
	if (cert.value && cert.value.pdfUrl) {
		...
		alert(
			"Bản in PDF chất lượng cao đang được khởi tạo hoặc chưa có link. Vui lòng liên hệ Admin.",
		);
	}
};
</script>
