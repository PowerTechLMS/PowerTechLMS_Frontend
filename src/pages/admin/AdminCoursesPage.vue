...
const deleteCourse = async (id: number) => {
	if (
		confirm(
			"Bạn có chắc chắn muốn xóa khóa học này không? Dữ liệu không thể khôi phục!",
		)
	) {
		try {
			...
			<div class="header-inner">
				...
				<div class="header-info">
					<div class="d-flex align-items-center gap-2 mb-1">
						...
						<span class="breadcrumb-mini"
							><ChevronRight :size="12" /> Danh lục khóa học</span
						>
					</div>
					<h1 class="title-gradient">Quản Lý Khóa Học</h1>
					<p class="desc-text">
						Không gian dành cho Admin phân bổ và kiểm soát chất lượng giáo
						trình.
					</p>
				</div>
			</div>
		...
		<div class="courses-main-container">
			...
			<div v-else-if="courses.length === 0" class="empty-glass-state py-5">
				...
				<p class="text-tertiary mb-4">
					Không tìm thấy dữ liệu phù hợp hoặc chưa có khóa học nào được tạo.
				</p>
				...
				>
					<div class="card-image-wrap">
						...
						<div class="badge-stack">
							<div
								class="status-badge"
								:class="course.isPublished ? 'published' : 'draft-warning'"
							>
								<CheckCircle2 v-if="course.isPublished" :size="12" />
								<Info v-else :size="12" />
								{{ course.isPublished ? "ĐÃ XUẤT BẢN" : "BẢN NHÁP (ẨN)" }}
							</div>
							<div
					...
					<div class="card-body-content">
						<h3 class="course-title-card" :title="course.title">
							{{ course.title }}
						</h3>
						...
						<div class="footer-meta mt-3 pt-3 border-top">
							<div class="instructor-tag">
								<div class="i-avatar">
									{{ course.createdByName?.charAt(0) || "A" }}
								</div>
								<div class="i-info">
					...
					<div class="card-action-bar-tri">
						<RouterLink
							:to="`/admin/courses/${course.id}/about`"
							target="_blank"
							class="act-btn"
							title="Xem dạng học viên"
							><Eye :size="18"
						/></RouterLink>
						<RouterLink
