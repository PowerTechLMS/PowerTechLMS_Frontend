...
// @ts-ignore
import { userGroupAPI, groupAPI } from '@/services/api'; // Đảm bảo import đúng API
import { toast } from 'vue3-toastify';
import { Building2, AlignLeft, Info, ArrowLeft, Save, X, Loader2, BookOpen, Check } from 'lucide-vue-next';
...
const form = ref({
  name: '',
  description: '',
  selectedCourseGroupIds: [] as number[] // MỚI: Mảng lưu ID Khung đào tạo
});
...
// MỚI: Fetch danh sách Khung đào tạo (Course Groups)
const fetchCourseGroups = async () => {
  isLoadingCourseGroups.value = true;
  try {
    const res = await groupAPI.getCourseGroups(1, 100); // Lấy toàn bộ Course Groups
    availableCourseGroups.value = res.data.items || res.data;
  } catch (error) {
    console.error("Lỗi khi tải Khung đào tạo:", error);
...
// MỚI: Xử lý chọn/bỏ chọn Khung đào tạo
...
const submitForm = async () => {
  ...
  try {
    ...
    // 2. Gán Khung đào tạo vào Phòng ban vừa tạo
    if (form.value.selectedCourseGroupIds.length > 0) {
        // Chạy song song các request gán Khóa học
        const assignPromises = form.value.selectedCourseGroupIds.map(cgId => 
            // Giả định bạn có một API endpoint để gán CourseGroup vào UserGroup (Department)
            // Ví dụ: userGroupAPI.assignCourseGroup(newDepartmentId, cgId)
            // LƯU Ý: Bạn cần tạo API endpoint tương ứng bên Backend C# cho tính năng này
    ...
    <div class="page-header">
      <div class="header-content">
        ...
        <div>
          ...
          <p class="page-desc">Tạo một bộ phận mới trong công ty để phân bổ lộ trình khóa học.</p>
        </div>
      </div>
    ...
    <div class="glass-card form-container">
      ...
      <div class="glass-card-banner">
        <div class="banner-icon-bg">
           <Building2 :size="20" style="width: 20px; height: 20px; min-width: 20px;" />
        </div>
        <div class="banner-text">
           <h5 class="banner-title">Cơ cấu tổ chức</h5>
           <p class="banner-desc">Thiết lập thông tin định danh cho phòng ban này.</p>
        </div>
      </div>
          ...
          <div class="form-grid">
            ...
            <div class="full-width-col">
               <div class="glass-alert-info">
                  <div class="alert-icon"><Info :size="20" style="width: 20px; height: 20px; min-width: 20px;" /></div>
                  <div class="alert-content">
                      <strong>Lưu ý:</strong> Khi bạn chọn <em>Khung lộ trình đào tạo</em>, toàn bộ nhân sự được thêm vào phòng ban này sẽ tự động được ghi danh vào các khóa học thuộc lộ trình đó.
                  </div>
               </div>
            </div>
            ...
            <div class="full-width-col input-wrapper">
              ...
              <div class="glass-input-group">
                <span class="input-icon"><Building2 :size="18" style="width: 18px; height: 18px; min-width: 18px;" /></span>
                <input 
                  type="text" 
                  class="glass-input" 
            ...
            <div class="full-width-col input-wrapper">
              ...
              <div class="glass-input-group textarea-group">
                <span class="input-icon"><AlignLeft :size="18" style="width: 18px; height: 18px; min-width: 18px;" /></span>
                <textarea 
                  class="glass-input" 
                  rows="3" 
            ...
            <div class="full-width-col input-wrapper mt-2 border-top-glass pt-4">
               ...
               <div v-if="isLoadingCourseGroups" class="d-flex align-items-center gap-2 text-tertiary fs-14">
                  <Loader2 :size="16" class="spinner-icon" /> Đang tải danh sách lộ trình...
               </div>
               
               <div v-else-if="availableCourseGroups.length === 0" class="text-muted fs-14 italic">
                  Chưa có khung lộ trình nào trong hệ thống. Bạn có thể gán sau.
               </div>

               <div v-else class="course-groups-grid">
                  <div v-for="cg in availableCourseGroups" :key="cg.id" 
                     ...
                     <div class="cg-info">
                        ...
                        <p class="cg-desc text-truncate" :title="cg.description">{{ cg.description || 'Chưa có mô tả' }}</p>
                        <span class="cg-count badge-glass fs-11">{{ cg.courseCount || 0 }} khóa học</span>
                     </div>
                  </div>
               </div>
            </div>
            ...
            <div class="full-width-col form-actions-container">
              <div class="action-buttons-wrapper">
                ...
                <button type="submit" class="btn-submit" :disabled="isSubmitting">
                   <Loader2 v-if="isSubmitting" :size="18" class="spinner-icon" style="width: 18px; height: 18px; min-width: 18px;" />
                   <Save v-else :size="18" style="width: 18px; height: 18px; min-width: 18px;" />  
...
.banner-title { margin: 0 0 4px 0; font-size: 16px; font-weight: 700; color: var(--text-primary); }
.banner-desc { margin: 0; font-size: 14px; color: var(--text-secondary); line-height: 1.4; }

.glass-card-body { padding: 40px 32px; }
...
/* ===== Glass Info Box ===== */
.glass-alert-info { display: flex; gap: 16px; align-items: flex-start; padding: 16px 20px; background: rgba(99, 102, 241, 0.05); border-radius: 12px; border: 1px solid rgba(99, 102, 241, 0.1); color: var(--primary-700); }
.alert-icon { flex-shrink: 0; margin-top: 2px; color: var(--primary-600); }
.alert-content { font-size: 14px; line-height: 1.6; }
.alert-content strong { font-weight: 700; color: var(--primary-800); }
.alert-content em { font-style: normal; font-weight: 600; text-decoration: underline; text-decoration-color: var(--primary-300); }

/* ===== Premium Inputs ===== */
.input-wrapper { display: flex; flex-direction: column; gap: 8px; }
.premium-label { font-weight: 700; font-size: 13px; text-transform: uppercase; letter-spacing: 0.05em; color: var(--text-secondary); }
...
.input-icon { position: absolute; left: 16px; color: var(--text-tertiary); z-index: 2; transition: color 0.3s; display: flex; align-items: center; justify-content: center; height: 100%; }
...
.glass-input { width: 100%; padding: 14px 16px 14px 46px; background: transparent; border: none; border-radius: 12px; color: var(--text-primary); font-size: 15px; font-family: inherit; line-height: 1.5; outline: none; z-index: 1; resize: none; }
.glass-input::placeholder { color: var(--text-tertiary); }

/* ===== Course Groups Grid UI (MỚI) ===== */
.course-groups-grid {
  display: grid;
...
.cg-info { flex: 1; overflow: hidden; }
.cg-name { margin: 0 0 4px 0; font-size: 14px; font-weight: 700; color: var(--text-primary); }
.cg-desc { margin: 0 0 8px 0; font-size: 12px; color: var(--text-tertiary); }
.badge-glass { display: inline-block; padding: 2px 8px; border-radius: 6px; font-weight: 700; background: rgba(0,0,0,0.05); color: var(--text-secondary); border: 1px solid rgba(0,0,0,0.05); }

/* ===== Action Buttons ===== */
.form-actions-container { padding-top: 32px; margin-top: 8px; border-top: 1px solid rgba(0,0,0,0.05); display: flex; justify-content: flex-end; }
.action-buttons-wrapper { display: flex; gap: 16px; }

.btn-cancel, .btn-submit { display: inline-flex; align-items: center; justify-content: center; gap: 8px; height: 48px; padding: 0 28px; border-radius: var(--radius-full); font-weight: 700; font-size: 15px; cursor: pointer; transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1); }
...
@media (max-width: 768px) {
  ...
  .glass-card-banner, .glass-card-body { padding: 20px; }
  .course-groups-grid { grid-template-columns: 1fr; }
  .action-buttons-wrapper { flex-direction: column-reverse; width: 100%; }
