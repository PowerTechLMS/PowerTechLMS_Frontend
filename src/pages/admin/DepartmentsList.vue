	...
	<div class="departments-page">
		<div class="page-header">
			<div class="header-content">
				...
				<div>
					...
					<p class="page-desc">
						Quản lý cơ cấu nhân sự, tự động phân bổ khung lộ trình đào tạo theo
						từng bộ phận.
					</p>
				</div>
			</div>
		...
		<div class="stats-grid">
			<div class="glass-stat-card">
				...
				<div class="stat-info">
					...
					<h3 class="stat-value">
						{{ stats.total }} <span class="stat-unit">bộ phận</span>
					</h3>
				</div>
			</div>
			...
			<div class="glass-stat-card">
				...
				<div class="stat-info">
					...
					<h3 class="stat-value">
						{{ stats.totalMembers }} <span class="stat-unit">nhân viên</span>
					</h3>
				</div>
			</div>
		</div>
		...
		<div class="content-card glass-card">
			...
			<div v-else-if="filteredDepartments.length === 0" class="empty-state">
				...
				<p>
					Thêm phòng ban mới để bắt đầu phân bổ nhân sự và tự động hóa lộ trình
					đào tạo.
				</p>
			</div>
						...
						<tr v-for="dept in paginatedDepartments" :key="dept.id">
							<td class="text-center">
								<span class="dept-id-badge"
									>PB-{{ String(dept.id).padStart(3, "0") }}</span
								>
							</td>
							...
							<td class="text-center">
								<div
									class="course-group-badge mx-auto"
									title="Số lượng khung lộ trình được tự động áp dụng"
								>
							...
							<td>
								<div
									class="fs-13 text-secondary d-flex align-items-center gap-2 fw-medium"
								>
							...
							<td>
								<div class="action-buttons">
									...
									<button
										...
										title="Giải thể phòng ban"
										@click="deleteDepartment(dept)"
										:disabled="deletingId === dept.id"
									>
										<Loader2
											v-if="deletingId === dept.id"
											:size="16"
											class="spinner-icon"
										/>
			...
			<div v-if="!loading && totalPages > 1" class="pagination-container">
				<div class="pagination-info">
					...
					trong tổng <strong>{{ filteredDepartments.length }}</strong> bộ phận
				</div>
...
const stats = computed(() => ({
	...
	totalMembers: departments.value.reduce(
		(sum: number, d: any) => sum + (d.memberCount || 0),
		0,
	),
}));
...
const totalPages = computed(
	() => Math.ceil(filteredDepartments.value.length / itemsPerPage.value) || 1,
);
...
const deleteDepartment = async (dept: any) => {
	if (
		confirm(
			`Phòng ban "${dept.name}" đang có ${dept.memberCount} nhân viên. Giải thể phòng ban này sẽ không xóa nhân sự nhưng sẽ gỡ họ khỏi các lộ trình học tập thuộc phòng. Bạn chắc chắn chứ?`,
		)
	) {
