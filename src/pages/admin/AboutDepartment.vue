...
const memberProgresses = ref<any>({});
...
const fetchDetails = async () => {
	...
	try {
		...
		const enrollRes = await enrollmentAPI
			.getAll({ pageSize: 5000 })
			.catch(() => ({ data: { items: [] } }));
		...
		if (department.value.members) {
			department.value.members.forEach((m: any) => {
				const userCourses = allEnrollments.filter(
					(e: any) => e.userId === m.userId,
				);
				...
				if (userCourses.length > 0) {
					const totalProgress = userCourses.reduce(
						(sum: number, course: any) => sum + (course.progressPercent || 0),
						0,
					);
...
const stats = computed(() => {
	...
	if (total === 0)
		return { total: 0, avgProgress: 0, completed: 0, learning: 0 };
...
const totalPages = computed(
	() => Math.ceil(filteredMembers.value.length / itemsPerPage.value) || 1,
);
		...
		<template v-else-if="department">
			...
			<div class="row g-4 mb-4 align-items-stretch">
				<div class="col-xl-7 col-lg-6">
					<div
						class="glass-hero-card h-100 position-relative overflow-hidden text-white"
					>
						<div class="glass-bg-accent"></div>

						<div
							class="card-body p-4 p-md-5 d-flex flex-column justify-content-between h-100 z-1 position-relative"
						>
							<div>
								<span
									class="badge-glass-white mb-3 text-uppercase tracking-wide"
								>
									<Building2
										:size="12"
										class="me-1 mb-1"
								...
								<p class="hero-desc">
									{{
										...
										"Chưa có mô tả định danh cho bộ phận này."
									}}
								</p>

								<div
									class="assigned-cgs mt-4 pt-4 border-top border-light border-opacity-25"
								>
									<h6
										class="fw-bold mb-3 fs-13 text-white opacity-75 d-flex align-items-center text-uppercase tracking-wide"
									>
										<BookOpen :size="16" class="me-2" /> Lộ trình đào tạo tự
										động:
									</h6>
									<div
										v-if="assignedCourseGroups.length === 0"
										class="fs-13 opacity-50 fst-italic"
									>
										Chưa gán lộ trình nào.
									</div>
									<div v-else class="d-flex flex-wrap gap-2">
										<span
											v-for="cg in assignedCourseGroups"
											:key="cg.id"
											class="badge-glass-white outline"
										>
											{{ cg.name }}
										</span>
									</div>
							...
							<div class="hero-metadata mt-4">
								...
								<div>
									...
									<span class="fs-13 opacity-75 d-block"
										>Mã định danh ID:
										<strong>#{{ department.id }}</strong> &bull; Tạo ngày:
										{{
				...
				<div class="col-xl-5 col-lg-6">
					<div class="row g-3 h-100">
						<div class="col-sm-6">
							<div
								class="glass-stat-card h-100 p-4 position-relative overflow-hidden"
							>
			...
			<div class="content-card glass-card mb-5">
					...
					<table class="glass-table custom-table mb-0">
						...
						<tbody>
							...
							<tr v-for="(m, idx) in paginatedMembers" :key="m.userId">
								...
								<td>
									<div class="d-flex align-items-center gap-3">
										<img
											...
											class="rounded-circle avatar-glass"
											width="42"
											height="42"
										/>
										<div class="lh-sm">
								...
								<td>
									<div
										class="d-flex justify-content-between fs-11 fw-bold mb-2"
									>
										<span
											:class="
												memberProgresses[m.userId]?.avgProgress === 100
													? 'text-success'
													: 'text-primary'
											"
										>
											...
											<span class="text-muted fw-normal"
												>({{
													memberProgresses[m.userId]?.totalAssigned || 0
												}}
												môn)</span
											>
										</span>
										<span class="text-muted d-flex align-items-center">
											...
											{{
												m.addedAt
													? new Date(m.addedAt).toLocaleDateString("vi-VN")
													: "---"
											}}
										</span>
									</div>
								...
								<td class="text-center">
									<span
										...
										:class="{
											...
											'text-primary bg-primary bg-opacity-10 border border-primary border-opacity-25':
												memberProgresses[m.userId]?.status === 'Đang học',
											'text-success bg-success bg-opacity-10 border border-success border-opacity-25':
												memberProgresses[m.userId]?.status === 'Hoàn thành',
										}"
									>
										{{ memberProgresses[m.userId]?.status || "Chưa bắt đầu" }}
									</span>
								...
								<td class="text-center pe-3">
									<button
										class="btn-ghost-icon view w-auto px-3 py-1 rounded-pill"
										@click="viewProfile(m.userId)"
				...
				<div class="glass-footer">
					<p class="text-muted fs-13 mb-3">
						Chỉ có quản trị viên mới có quyền thêm/bớt nhân sự trực thuộc bộ
						phận.
					</p>
