...
import { 
  Route, ArrowLeft, BookOpen, Save, Settings, 
  Plus, Trash2, Loader2, GripVertical, CheckCircle
...
const activeTab = ref('courses'); // Đặt tab Khóa học làm mặc định
    ...
    <div v-else>
      <div class="page-header mb-4">
        <div class="header-content">
          <div class="header-icon-box pulse-glow"><Route :size="32" style="width: 32px; height: 32px;" /></div>
          <div>
            <span class="badge-glass primary mb-1">MÃ: PATH-{{ String(pathId).padStart(3, '0') }}</span>
            <h1 class="page-title-gradient">Cấu hình: {{ form.name }}</h1>
            <p class="page-desc">Quản lý danh sách khóa học thuộc lộ trình này.</p>
          </div>
        </div>
        <button @click="router.push('/admin/coursegroup')" class="btn btn-outline">
      ...
      <div class="glass-content-card">
        <div class="tabs-glass-nav">
          <button class="tab-glass-btn" :class="{ active: activeTab === 'courses' }" @click="activeTab = 'courses'">
            <BookOpen :size="18" class="me-2" /> Cấu trúc Khóa học <span class="tab-badge ms-2">{{ pathCourses.length }}</span>
          </button>
          <button class="tab-glass-btn" :class="{ active: activeTab === 'general' }" @click="activeTab = 'general'">
            <Settings :size="18" class="me-2" /> Thông tin chung
          </button>
        </div>
          ...
          <div v-show="activeTab === 'general'" class="fade-in">
            <form @submit.prevent="updatePath" style="max-width: 600px;">
              <div class="form-group mb-4">
                <label class="premium-label">Tên Lộ trình <span class="text-danger">*</span></label>
                <input type="text" class="glass-input w-100" v-model="form.name" required>
              </div>
              <div class="form-group mb-4">
                <label class="premium-label">Mô tả mục tiêu đào tạo</label>
                <textarea class="glass-input w-100" rows="5" v-model="form.description"></textarea>
              </div>
              <div class="border-top-glass pt-4 mt-2">
          ...
          <div v-show="activeTab === 'courses'" class="fade-in">
            <div class="glass-add-box mb-4">
              <div class="flex-grow-1">
                <label class="premium-label text-primary mb-2">Thêm khóa học vào lộ trình này</label>
              ...
              <button class="btn btn-primary" @click="addCourseToPath" :disabled="!selectedCourseToAdd || isSubmitting">
              ...
              <table class="glass-table custom-table mb-0 w-100">
                ...
                <tbody>
                  <tr v-if="pathCourses.length === 0">
                    <td colspan="4" class="text-center py-5 text-muted">Lộ trình này chưa có khóa học nào. Vui lòng thêm ở trên.</td>
                  </tr>
                  <tr v-for="(course, idx) in pathCourses" :key="course.id">
                    <td class="text-center text-muted fw-bold">
                      <div class="d-flex align-items-center justify-content-center gap-1">
                        <GripVertical :size="14" class="opacity-25" /> {{ idx + 1 }}
                      </div>
                    </td>
                    <td>
                      <div class="fw-bold text-dark fs-14">{{ course.title }}</div>
                      <div class="text-muted fs-12 mt-1">Mã khóa học: #{{ course.id }}</div>
                    </td>
                    <td class="text-center">
                      <span class="badge-glass success"><CheckCircle :size="12" class="me-1"/> Đã liên kết</span>
                    </td>
                    <td class="text-center">
                      <button class="btn-ghost-icon delete" @click="removeCourse(course.id, course.title)">
                        <Trash2 :size="18" />
                      </button>
                    </td>
                  </tr>
...
.edit-path-page { padding-bottom: var(--space-2xl); animation: fadeIn 0.4s ease-out; max-width: 1100px; margin: 0 auto; color: var(--text-primary); }
.fade-in { animation: fadeIn 0.3s ease-in-out; }

.page-header { display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 24px; border-bottom: 1px solid rgba(0,0,0,0.05); flex-wrap: wrap; gap: var(--space-lg); }
.header-content { display: flex; align-items: flex-start; gap: var(--space-xl); }
.header-icon-box { width: 64px; height: 64px; border-radius: 20px; background: linear-gradient(135deg, rgba(99, 102, 241, 0.15), rgba(99, 102, 241, 0.05)); color: var(--primary-500); display: flex; align-items: center; justify-content: center; border: 1px solid rgba(99, 102, 241, 0.2); flex-shrink: 0; }
...
.badge-glass { display: inline-block; padding: 4px 12px; border-radius: var(--radius-full); font-size: 11px; font-weight: 700; letter-spacing: 0.05em; border: 1px solid transparent; }
.badge-glass.primary { background: rgba(99, 102, 241, 0.1); color: var(--primary-600); border-color: rgba(99, 102, 241, 0.2); }
.badge-glass.success { background: rgba(16, 185, 129, 0.1); color: var(--success-700); border-color: rgba(16, 185, 129, 0.2); display: flex; align-items: center; justify-content: center; width: max-content; margin: 0 auto;}

.page-title-gradient { font-size: 28px; font-weight: 800; letter-spacing: -0.02em; background: linear-gradient(90deg, var(--text-primary), var(--text-secondary)); -webkit-background-clip: text; -webkit-text-fill-color: transparent; margin: 0 0 4px 0; }
.page-desc { font-size: 14px; color: var(--text-secondary); margin: 0; }

.btn { display: inline-flex; align-items: center; gap: 8px; padding: 10px 20px; border-radius: var(--radius-full); font-weight: 600; font-size: 14px; cursor: pointer; transition: all 0.3s; border: none; }
...
.tabs-glass-nav { display: flex; gap: 8px; padding: 12px 24px; border-bottom: 1px solid rgba(0,0,0,0.05); background: rgba(249, 250, 251, 0.5); overflow-x: auto; }
...
.glass-input { width: 100%; padding: 12px 16px; background: rgba(249, 250, 251, 0.5); border: 1px solid rgba(0, 0, 0, 0.06); border-radius: 12px; color: var(--text-primary); font-size: 14px; transition: all 0.3s; outline: none; }
.glass-input:focus { background: white; border-color: var(--primary-400); box-shadow: 0 0 0 4px rgba(99, 102, 241, 0.1); }

.glass-add-box { background: rgba(99,102,241,0.03); border: 1px dashed rgba(99,102,241,0.3); border-radius: 16px; padding: 20px; display: flex; gap: 16px; align-items: flex-end; flex-wrap: wrap; }
...
.glass-table th { text-align: left; padding: 12px 16px; font-size: 11px; font-weight: 700; color: var(--text-tertiary); text-transform: uppercase; border-bottom: 2px solid rgba(0,0,0,0.05); background: rgba(249, 250, 251, 0.5); }
.glass-table td { padding: 16px; border-bottom: 1px solid rgba(0,0,0,0.03); vertical-align: middle; }

.btn-ghost-icon { display: inline-flex; align-items: center; justify-content: center; width: 36px; height: 36px; border-radius: 10px; background: transparent; border: 1px solid transparent; cursor: pointer; transition: all 0.2s; color: var(--text-tertiary); }
.btn-ghost-icon.delete:hover { color: var(--danger-600); background: rgba(239, 68, 68, 0.1); border-color: rgba(239, 68, 68, 0.2); }
