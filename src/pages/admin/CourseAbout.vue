  ...
  <div class="course-about-premium">
    <div class="page-header-premium animate-fade-in">
      ...
      <div class="header-actions">
        <ol class="breadcrumb-glass">
          <li class="breadcrumb-item"><RouterLink to="/admin/courses">Danh sách</RouterLink></li>
      ...
      <div class="row">
        <div class="col-lg-8">
          ...
          <div class="skeleton-content glass p-4">
            ...
            <div v-for="i in 3" :key="i" class="skeleton-module-row glass mb-3"></div>
          </div>
        </div>
        <div class="col-lg-4">
    ...
    <div v-else-if="courseData" class="container-fluid mt-4 pb-5">
      <div class="row g-4">
        <div class="col-xl-8 col-lg-8 animate-slide-up">
          <div class="preview-main-card glass overflow-hidden shadow-glow">
            <div class="preview-banner-container">
              <img v-if="courseData.coverImageUrl" :src="getImageUrl(courseData.coverImageUrl)" class="preview-banner" />
              ...
              <div class="banner-badge">
                <span class="badge-glass" :class="courseData.isPublished ? 'success' : 'warning'">
            ...
            <div class="preview-body p-4 p-md-5">
              <h2 class="preview-title mb-4 fw-black title-gradient">{{ courseData.title }}</h2>
              ...
              <div class="sylllabus-section mt-5">
                <div class="d-flex justify-content-between align-items-center mb-4">
                  <h4 class="syllabus-title h5 mb-0"><CheckCircle :size="18" class="text-primary me-2" />Giáo trình chi tiết</h4>
                  <span class="badge-glass dark">{{ syllabus.length }} Chương</span>
                </div>
                
                <div class="accordion-syllabus">
                  <div v-for="(module, index) in syllabus" :key="module.id" class="module-glass-card mb-3" :class="{ 'expanded': isModuleExpanded(module.id, index) }">
                    <div class="module-header p-4" @click="toggleModule(module.id)">
                      <div class="d-flex justify-content-between align-items-center w-100">
                        ...
                        <div class="d-flex align-items-center gap-3">
                          <span class="badge-glass primary sm">{{ module.lessons.length }} Bài</span>
                          <ChevronUp v-if="isModuleExpanded(module.id, index)" :size="18" class="text-primary" />
                          <ChevronDown v-else :size="18" class="text-secondary" />
                        </div>
                      </div>
                    </div>
                    
                    <div v-show="isModuleExpanded(module.id, index)" class="module-content-wrap">
                      <div class="lessons-list">
                        <div v-for="lesson in module.lessons" :key="lesson.id" class="lesson-glass-item">
                          <div class="lesson-info-box">
                            <div class="lesson-type-icon">
                              <PlayCircle v-if="lesson.type === 'Video'" :size="18" class="text-primary" />
                              <FileText v-else :size="18" class="text-info" />
                            </div>
                            <div class="lesson-text">
                              <span class="lesson-title fw-semibold">{{ lesson.title }}</span>
                              <div class="d-flex gap-2 mt-1 flex-wrap">
                                <span v-if="lesson.documents.length > 0" class="doc-tag">
                          ...
                          <div class="lesson-actions-box">
                            <button v-if="lesson.isFreePreview && lesson.type === 'Video' && lesson.videoUrl" class="badge-neon success me-3" @click="openPreview(lesson)">HỌC THỬ</button>
                            <span class="lesson-duration"><Clock :size="14" /> {{ formatDuration(lesson.durationSeconds) }}</span>
                          </div>
                        </div>
                      </div>
                    </div>
        ...
        <div class="col-xl-4 col-lg-4 animate-slide-up" style="animation-delay: 0.1s;">
          <div class="sidebar-sticky">
            <div class="stat-card-premium glass p-4 shadow-glow">
              <div class="price-header text-center mb-4">
                ...
                <div class="text-muted small fw-bold">DÀNH CHO NHÂN VIÊN HỆ THỐNG</div>
              </div>
              ...
              <div class="info-list-glass">
                <div class="info-row">
                  ...
                  <div class="info-value fw-bold text-primary">{{ courseData.categoryName || 'Chưa phân loại' }}</div>
                </div>
                <div class="info-row">
                ...
                <div class="info-row">
                  ...
                  <div class="info-value">{{ courseData.enrollmentCount || 0 }}</div>
                </div>
                <div class="info-row">
                  <div class="info-label"><User :size="16" /> Giảng viên</div>
                  <div class="info-value">{{ courseData.createdByName || 'Admin' }}</div>
                </div>
                <div class="info-row no-border">
                  <div class="info-label"><CheckCircle :size="16" /> Đặc quyền</div>
                  <div class="info-value text-success fw-bold">Chứng nhận hoàn thành</div>
                </div>
              </div>
    ...
    <Transition name="modal">
      <div v-if="previewData.videoUrl" class="glass-modal-overlay" @click.self="previewData.videoUrl = ''">
        <div class="glass-modal-container glass shadow-lg">
          <div class="glass-modal-header p-3 px-4">
            <div class="d-flex align-items-center gap-2">
              ...
              <h5 class="mb-0 fw-black small-caps text-white">{{ previewData.title }}</h5>
            </div>
            <button class="btn-close-glass" @click="previewData.videoUrl = ''"><X :size="20" /></button>
          </div>
          <div class="glass-modal-body bg-black">
            <div class="ratio ratio-16x9">
              <iframe v-if="previewData.isYoutube" :src="previewData.videoUrl" allowfullscreen></iframe>
              <video v-else :src="previewData.videoUrl" controls autoplay></video>
            </div>
          </div>
        </div>
      </div>
...
const totalDurationFormatted = computed(() => {
  const totalSeconds = syllabus.value.reduce((acc, mod) => {
    return acc + mod.lessons.reduce((lAcc, l) => lAcc + (l.durationSeconds || 0), 0);
  }, 0);
...
const openPreview = (lesson: Lesson) => {
  ...
  previewData.value.isYoutube = lesson.videoUrl.includes('youtube') || lesson.videoUrl.includes('youtu.be');
};
...
// Expand/collapse accordion state
...
<style scoped>
.course-about-premium { min-height: 100vh; }
...
.badge-glass.sm { scale: 0.85; transform-origin: center right; }
...
.breadcrumb-glass {
  display: flex; gap: 8px; list-style: none; margin: 0; padding: 6px 16px;
  background: rgba(255,255,255,0.6); border-radius: 30px; border: 1px solid white;
}
.breadcrumb-item a { color: #64748b; font-weight: 600; text-decoration: none; }
.breadcrumb-item.active { color: #1e293b; font-weight: 800; }

.preview-main-card { border-radius: 28px; }
.preview-banner-container { height: 380px; position: relative; width: 100%; }
.preview-banner { width: 100%; height: 100%; object-fit: cover; }
.preview-banner-placeholder {
  width: 100%; height: 100%; background: #f1f5f9; display: flex; flex-direction: column;
  align-items: center; justify-content: center; gap: 1rem; color: #94a3b8; font-weight: 700;
}
.banner-overlay { position: absolute; inset: 0; background: linear-gradient(180deg, transparent 50%, rgba(0,0,0,0.6) 100%); }
.banner-badge { position: absolute; top: 20px; right: 20px; z-index: 2; }
.preview-title { font-size: 2.25rem; line-height: 1.2; letter-spacing: -1px; }
.section-tag { font-size: 11px; font-weight: 900; color: #94a3b8; letter-spacing: 2px; }
.preview-desc { font-size: 1.05rem; line-height: 1.7; font-weight: 500; }

.module-glass-card {
  border-radius: 20px; border: 1px solid rgba(0,0,0,0.04); background: rgba(255,255,255,0.4);
  overflow: hidden; transition: all 0.3s cubic-bezier(0.4, 0, 0.2, 1);
}
.module-glass-card.expanded { background: white; border-color: rgba(79, 70, 229, 0.2); box-shadow: 0 10px 30px rgba(0,0,0,0.02); }
.module-header { cursor: pointer; transition: background 0.2s; }
.module-header:hover { background: rgba(255,255,255,0.8); }

.module-number {
  width: 32px; height: 32px; border-radius: 10px; background: #f1f5f9; color: #1e293b;
  display: flex; align-items: center; justify-content: center; font-weight: 900; font-size: 13px;
}

.lesson-glass-item {
  display: flex; justify-content: space-between; align-items: center;
  padding: 1.25rem 2rem; border-top: 1px solid rgba(0,0,0,0.03); transition: all 0.2s;
}
.lesson-glass-item:hover { background: rgba(79, 70, 229, 0.02); padding-left: 2.5rem; }

.lesson-info-box { display: flex; gap: 1rem; align-items: flex-start; }
.lesson-type-icon { margin-top: 2px; }
.lesson-text { display: flex; flex-direction: column; }
.lesson-title { color: #334155; font-size: 0.95rem; }

.doc-tag { font-size: 0.75rem; color: #64748b; font-weight: 700; background: #eff6ff; padding: 2px 8px; border-radius: 6px; }
...
.lesson-actions-box { display: flex; align-items: center; }
.lesson-duration { color: #94a3b8; font-size: 0.8rem; font-weight: 700; display: flex; align-items: center; gap: 4px; }

.sidebar-sticky { position: sticky; top: 100px; }
.stat-card-premium { border-radius: 28px; border: 1px solid white; }
.info-list-glass { display: flex; flex-direction: column; gap: 4px; }
...
.info-label { display: flex; align-items: center; gap: 10px; color: #64748b; font-weight: 700; font-size: 0.85rem; }
.info-value { color: #1e293b; font-weight: 800; font-size: 0.9rem; }

.btn-neon {
  padding: 0.9rem 1.5rem; border-radius: 16px; font-weight: 900; font-size: 0.9rem;
  border: none; color: white; background: linear-gradient(135deg, #6366f1, #4f46e5);
  box-shadow: 0 10px 20px rgba(79, 70, 229, 0.3); display: flex; align-items: center; justify-content: center; gap: 12px;
  transition: all 0.3s;
}
.btn-neon:hover { transform: translateY(-3px); box-shadow: 0 15px 30px rgba(79, 70, 229, 0.4); }

.glass-modal-overlay {
  position: fixed; inset: 0; z-index: 10000; background: rgba(15, 23, 42, 0.85); backdrop-filter: blur(25px);
  display: flex; align-items: center; justify-content: center; padding: 20px;
}
.glass-modal-container { width: 100%; max-width: 900px; border-radius: 30px; overflow: hidden; background: #0f172a; border: 1px solid rgba(255,255,255,0.1); }
.glass-modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.05); }
.btn-close-glass {
  background: rgba(255,255,255,0.1); border: none; color: white; width: 36px; height: 36px; border-radius: 50%;
  display: flex; align-items: center; justify-content: center; transition: all 0.2s;
}
.btn-close-glass:hover { background: #ef4444; }

.skeleton-banner { height: 380px; width: 100%; border-radius: 28px; background: linear-gradient(90deg, #f1f5f9 25%, #f8fafc 50%, #f1f5f9 75%); background-size: 200% 100%; animation: shimmer 1.5s infinite; }
.skeleton-line { height: 12px; border-radius: 6px; background: #e2e8f0; }
.skeleton-module-row { height: 70px; border-radius: 16px; background: #f1f5f9; }
.skeleton-sidebar { height: 450px; border-radius: 28px; background: #f1f5f9; }
...
.modal-enter-from, .modal-leave-to { opacity: 0; scale: 0.9; transform: translateY(20px); }
