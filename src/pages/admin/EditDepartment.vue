...
const initialCourseGroupIds = ref<number[]>([]);
...
const updateDepartment = async () => {
	...
	try {
		...
		toAdd.forEach((cgId) =>
			mappingPromises.push(userGroupAPI.assignCourseGroup(deptId, cgId)),
		);
		toRemove.forEach((cgId) =>
			mappingPromises.push(userGroupAPI.removeCourseGroup(deptId, cgId)),
		);
...
const addSelectedMembers = async () => {
	...
	isSubmitting.value = true;
	try {
		const promises = selectedUserIds.value.map((uid) =>
			userGroupAPI.addUser(deptId, uid),
		);
		await Promise.all(promises);
		toast.success(
			`Đã thêm ${selectedUserIds.value.length} nhân sự và gán lộ trình.`,
		);
...
const removeMember = async (userId: number, userName: string) => {
	if (
		!confirm(
			`Bạn có chắc muốn gỡ ${userName} khỏi nhóm? (Các khóa học hiện tại sẽ không bị xóa)`,
		)
	)
		return;
	try {
...
<template>
	<div class="edit-department-page">
		<div
			v-if="loading"
			class="loading-state h-50 d-flex flex-column align-items-center justify-content-center mt-5 pt-5"
		>
		...
		<div v-else>
			<div class="page-header">
				<div class="header-content">
					...
					<div>
						<span class="id-badge"
							>MÃ PB: PB-{{ String(deptId).padStart(3, "0") }}</span
						>
						<h1 class="page-title-gradient">Cập nhật: {{ form.name }}</h1>
						<p class="page-desc">
							Quản lý cơ cấu, lộ trình tự động và nhân sự trực thuộc.
						</p>
					</div>
				</div>
								...
								<div class="row g-5">
									<div class="col-lg-6 position-relative">
										...
										<div class="form-group mt-5">
											<label class="premium-label mb-3"
												>Trạng thái khối/phòng ban</label
											>
									...
									<div class="col-lg-6 border-start-glass pl-lg-4">
										<div class="glass-assignment-box">
											...
											<p class="ca-desc mb-3">
												Chọn các khung đào tạo. Bất kỳ nhân sự nào thuộc bộ phận
												này sẽ tự động được ghi danh vào tất cả khóa học của lộ
												trình được chọn.
											</p>

											<div
												v-if="courseGroups.length === 0"
												class="text-muted fs-14 italic text-center py-4"
											>
											...
											<div v-else class="course-groups-grid custom-scrollbar">
												<div
													v-for="cg in courseGroups"
													:key="cg.id"
													class="cg-select-card glass-hover"
													:class="{
														selected: form.selectedCourseGroupIds.includes(
															cg.id,
														),
													}"
												...
												>
													...
													<div class="cg-info">
														...
														<span class="cg-count badge-glass fs-11 mt-1"
							...
							<div class="row g-4">
								<div class="col-xl-4 col-lg-5">
									<div class="glass-add-member-box">
										<label
											class="premium-label text-primary mb-3 d-flex align-items-center"
										>
											<UserPlus
												:size="18"
												class="me-2"
										...
										<div class="glass-search w-100 mb-3">
											...
											<input
												...
												placeholder="Tìm người muốn gán..."
												v-model="userSearch"
											/>
										</div>
										...
										<div class="glass-user-list custom-scrollbar">
											<div
												...
												class="empty-list-text text-muted text-center py-4"
											>
										...
										<button
											class="btn-submit w-100 mt-4 justify-content-center"
								...
								<div class="col-xl-8 col-lg-7 d-flex flex-column">
									<div
										class="glass-section-header px-0 border-0 mb-3 bg-transparent"
									>
										<div class="badge-group mb-2 mb-sm-0">
											<span class="badge-glass primary"
												>Tất cả: {{ memberStats.total }}</span
											>
											<span class="badge-glass success"
												>Hoàn thành: {{ memberStats.done }}</span
											>
										</div>
												...
												<tr v-for="(m, idx) in filteredMembers" :key="m.userId">
													...
													<td>
														<div class="d-flex align-items-center gap-3">
															...
															<div class="user-info w-100">
																...
																<div
																	class="user-date mt-1 text-tertiary fs-11 d-flex align-items-center"
																>
																	<CalendarCheck :size="10" class="me-1" />Ngày
																	vào nhóm:
																	{{
																		m.addedAt
													...
													<td>
														...
														>
															...
															<span
																class="status-pill offline-pill py-1 px-2 fs-10"
																>Đang học</span
															>
														</div>
													...
													<td class="text-center pe-3">
														<button
															...
															@click="removeMember(m.userId, m.fullName)"
															title="Gỡ khỏi nhóm"
														>
															<Trash2 :size="16" />
														</button>
...
.glass-stat-card.info {
	border-bottom: 3px solid var(--info-500);
}
.glass-stat-card.warning {
...
.glass-stat-card.info .stat-icon-wrap {
	background: var(--info-50);
	color: var(--info-600);
}
.glass-stat-card.warning .stat-icon-wrap {
...
.tab-glass-btn.active .tab-badge {
	background: var(--primary-50);
...
.status-option.active .status-dot.danger {
